#!/usr/bin/env python3
"""
Test AES-GCM-SIV CLI-Mobile compatibility
"""
...
def test_aes_gcm_siv_cli_decryption():
    """Test decrypting CLI AES-GCM-SIV file with mobile crypto"""
    print("🧪 Testing AES-GCM-SIV CLI-Mobile Decryption")
    print("=" * 50)
    ...
    if result.get('success'):
        ...
        if decrypted.strip() == "Hello World":
            print("✅ SUCCESS: AES-GCM-SIV CLI-Mobile compatibility works!")
            return True
        else:
...
def test_aes_gcm_siv_mobile_encryption():
    """Test mobile AES-GCM-SIV encryption"""
    print("\n🧪 Testing Mobile AES-GCM-SIV Encryption")
    print("=" * 50)
    
    crypto = MobileCryptoCore()
    test_data = "Hello Mobile AES-GCM-SIV!"
    password = "testpass123"
    
    # Test encryption
    result = crypto.encrypt_data(test_data.encode(), password, "aes-gcm-siv")
    ...
    if result.get('success'):
        ...
        if decrypt_result.get('success'):
            ...
            if decrypted == test_data:
                print("✅ SUCCESS: Mobile AES-GCM-SIV round-trip works!")
                return True
            else:
...
def main():
    """Run all AES-GCM-SIV tests"""
    print("🎯 AES-GCM-SIV CLI-Mobile Compatibility Testing")
    print("=" * 60)
    
    success_count = 0
    total_tests = 2
    
    # Test 1: CLI AES-GCM-SIV decryption
    if test_aes_gcm_siv_cli_decryption():
        success_count += 1
    
    # Test 2: Mobile AES-GCM-SIV encryption/decryption
    if test_aes_gcm_siv_mobile_encryption():
        success_count += 1
    ...
    if success_count == total_tests:
        print("🎉 ALL TESTS PASSED - AES-GCM-SIV is working!")
        return True
    else:
