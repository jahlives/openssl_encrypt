...
from cryptography.hazmat.primitives.ciphers.aead import ChaCha20Poly1305, AESGCM, AESGCMSIV, AESOCB3, AESSIV
...
class MobileCryptoCore:
    ...
    def multi_hash_password(self, password: bytes, salt: bytes, hash_config: Dict[str, int] = None) -> bytes:
        """
    ...
    def multi_kdf_derive_with_algorithm(self, password: bytes, salt: bytes, kdf_config: Dict[str, Any] = None, algorithm: str = None) -> bytes:
        """
        ...
        # 1. Argon2 (if enabled)
        if "argon2" in kdf_config and kdf_config["argon2"].get("enabled", False) and ARGON2_AVAILABLE:
        ...
        if "balloon" in kdf_config and kdf_config["balloon"].get("enabled", False) and BALLOON_AVAILABLE:
            ...
            parallel_cost = balloon_params.get("parallelism", balloon_params.get("parallel_cost", 1))
    ...
    def _derive_key_with_algorithm(self, password: str, salt: bytes, hash_config: Dict[str, int] = None, 
                   kdf_config: Dict[str, Any] = None, algorithm: str = None) -> bytes:
        """
        ...
        derived_key = self.multi_kdf_derive_with_algorithm(hashed_password, salt, kdf_config, algorithm)
    ...
    def _derive_key(self, password: str, salt: bytes, hash_config: Dict[str, int] = None, 
                   kdf_config: Dict[str, Any] = None) -> bytes:
        """
    ...
    def encrypt_data(self, data: bytes, password: str, algorithm: str = "fernet", 
                    hash_config: Dict[str, int] = None, kdf_config: Dict[str, Any] = None, 
        ...
        try:
            ...
            if algorithm == "fernet":
                ...
                final_key = self.multi_kdf_derive_with_algorithm(raw_key, salt, kdf_config, algorithm)
    ...
    def decrypt_data(self, encrypted_data_b64: str, metadata: Dict[str, Any], password: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if metadata.get("format_version") in [4, 5] and "derivation_config" in metadata:
                ...
                if algorithm == "fernet":
                    ...
                    raw_key = self.multi_hash_password(password.encode(), salt, hash_config)
                    key = self.multi_kdf_derive(raw_key, salt, kdf_config)[:32]  # 32 bytes for AES-256
                elif algorithm == "aes-gcm-siv":
                    ...
                    raw_key = self.multi_hash_password(password.encode(), salt, hash_config)
                    key = self.multi_kdf_derive(raw_key, salt, kdf_config)[:32]  # 32 bytes for ChaCha20
                elif algorithm == "xchacha20-poly1305":
                    ...
                    if algorithm in ["xchacha20-poly1305", "aes-siv"]:
                        key = self._derive_key_with_algorithm(password, salt, hash_config, kdf_config, algorithm)
                    else:
                        key = self._derive_key(password, salt, hash_config, kdf_config)  # Default fallback
            ...
            # Handle old mobile format (direct salt)
                ...
                algorithm = metadata.get("encryption", {}).get("algorithm", "fernet")
                if algorithm in ["xchacha20-poly1305", "aes-siv"]:
                    key = self._derive_key_with_algorithm(password, salt, single_hash_config, legacy_kdf_config, algorithm)
                else:
    ...
    def get_hash_algorithms(self) -> str:
        ...
        cli_order = ["sha512", "sha256", "sha3_256", "sha3_512", "blake2b", "blake3", "shake256", "whirlpool"]
    ...
    def get_crypto_config(self) -> str:
        ...
        return json.dumps({
            ...
            "kdf_algorithms": [kdf for kdf in self.kdf_algorithms.keys() 
                             if kdf != "argon2" or ARGON2_AVAILABLE],
    ...
    def encrypt_file(self, file_path: str, password: str, output_path: str = None, progress_callback=None) -> Dict[str, Any]:
        """
    ...
    def decrypt_file(self, encrypted_file_path: str, password: str, output_path: str = None, progress_callback=None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if ':' in raw_content and not raw_content.startswith('{'):
                ...
                try:
                    ...
                    print(f"📋 Detected CLI format version {metadata.get('format_version', 'unknown')}")
                ...
                try:
                    ...
                    return {"success": False, "error": f"Failed to parse mobile JSON format: {str(e)}"}
