...
## Steps
- [ ] P1: Three separate confidential GitLab issues created (one per finding)
      per issue-tracking skill; each fix commit references its issue.
  target: gitlab world/openssl_encrypt
- [ ] P2: Baseline full test run recorded before any change (tdd-workflow).
  target: openssl_encrypt/unittests/
- [ ] P3: Regression test exists asserting the derive-password handler
      zeroizes the HSM pepper buffer (fails against current code).
  target: openssl_encrypt/unittests/
- [ ] P4: derive-password holds `hsm_pepper` in a `bytearray` and
      `secure_memzero`s it in a `finally` (mirror crypt_core.py:6380-6389).
  target: openssl_encrypt/modules/crypt_cli.py:4842
- [ ] P5: derive-password wipes the wipeable derived-key copies (`derived`
...
- [ ] P6: Changelog files updated for LOW-1 and fix committed (one commit,
...
- [ ] P7: Regression test exists asserting the no-hash-iterations legacy path
  ...
  target: openssl_encrypt/unittests/
- [ ] P8: The `password + salt + hsm_pepper` seed at crypt_core.py:3058 is
...
- [ ] P9: Changelog files updated for INFO-1 and fix committed (one commit,
...
- [x] P10: INFO-2 behavior decided (2026-07-13, user): version-gated
  ...
  target: (decision)
- [ ] P11: Tests exist covering (a) v14+ balloon config with `space_cost`
      ...
      passing).
  target: openssl_encrypt/unittests/
- [ ] P12: `compute_kdf_independent` implements the version-gated fail-closed
      behavior from P10 (needs format_version available at that point —
      verify plumbing).
  target: openssl_encrypt/modules/crypt_core.py:2095
- [ ] P13: Changelog files updated for INFO-2 and fix committed (one commit,
      references its P1 issue).
  target: CHANGELOG.md, version.py.template, metainfo.xml, changelog.html
- [ ] P14: Full test suite green at the feature boundary
      (pytest -n auto --dist=worksteal openssl_encrypt/unittests/).
  target: openssl_encrypt/unittests/
- [ ] P15: security-reviewer re-run confirms all three findings resolved with
...
- [ ] P17: `_kdf_worker` in parallel_kdf.py applies the same version-gated
      fail-closed behavior as P12 (same fallback pattern, found during
      archaeology), covered by a test.
