...
def patch_keystore_file():
    ...
    if (
        "from argon2 import PasswordHasher" in content
        and "from argon2.low_level import hash_secret_raw, Type" not in content
    ):
    ...
    type=Type.ID
        ...
        if first_pos >= 0:
            ...
            if second_pos >= 0:
                ...
                content = (
                    content[:second_full_pos]
                    + new_derivation
                    + content[second_full_pos + len(old_derivation) :]
                )
