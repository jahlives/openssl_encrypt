...
class WAVSteganography(SteganographyBase):
    ...
    def __init__(self, password: Optional[str] = None, security_level: int = 1,
                 bits_per_sample: int = 1, preserve_quality: bool = True,
        ...
        try:
            ...
            try:
                ...
                if wav_info['format'] != self.PCM_FORMAT:
                    raise CoverMediaError(f"Unsupported audio format: {wav_info['format']} (only PCM supported)")
                ...
                logger.debug(f"WAV capacity: {capacity} bytes ({total_samples} samples, {channels} channels)")
    ...
    def _analyze_wav_structure(self, wav_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            wav_info['total_samples'] = wav_info['data_size'] // (bytes_per_sample * wav_info['channels'])
    ...
    def _hide_in_wav_samples(self, samples: np.ndarray, secret_data: bytes, 
                            wav_info: Dict[str, Any]) -> np.ndarray:
        ...
        try:
            ...
            data_to_hide = SecureBytes(length_bytes + secret_data + b'\x00\x01\x02\x03\x04\x05\x06\x07')  # End marker
    ...
    def _samples_to_wav_bytes(self, samples: np.ndarray, wav_info: Dict[str, Any], 
                             original_wav_data: bytes) -> bytes:
        ...
        while offset < len(wav_data) - 8:
            ...
            if chunk_id == b'data':
                ...
                if 'bits_per_sample' in audio_props and 'channels' in audio_props:
                    ...
                    audio_props['total_samples'] = chunk_size // (bytes_per_sample * audio_props['channels'])
    ...
    def _analyze_steganography_suitability(self, wav_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if format_code == 1:
                ...
                suitability['recommendations'].append("Compressed audio - limited steganography potential")
            else:
            ...
            if suitability['overall_score'] >= 0.9:
                ...
                suitability['recommendations'].append("WAV suitable with careful parameter selection")
...
def create_wav_test_audio(duration_seconds: float = 5.0, sample_rate: int = 44100, 
    ...
    try:
        ...
        # Generate waveform
        waveform = (np.sin(2 * np.pi * frequency1 * time_array) * 0.3 + 
