...
def setup_encrypt_parser(subparser):
    ...
    subparser.add_argument(
        "--random-password-out",
        metavar="PATH",
        help="Write the password generated by --random to PATH instead of "
        "displaying it. The file is created 0600 and the command refuses to "
        "overwrite an existing one. Required when stderr is not a terminal: "
        "the display is collapsed into stdout by 2>&1 and lands in scrollback "
        "and log files, and losing the password means losing the data.",
    )
    subparser.add_argument(
        "--force-password",
        action="store_true",
...
def setup_hsm_parser(subparser):
    ...
    # namespace and copies every key back over the parent's, so a default
    # here would overwrite a --yes relocated by preprocess_global_args. Same
    # dest-clobber --quiet hit in gitlab#171; --yes could not join
    # TRULY_GLOBAL_FLAGS until this was fixed (gitlab#176).
    fido2_unregister_parser.add_argument(
        "--yes",
        "-y",
        action="store_true",
