...
RandomX silent drop (#71), and a balloon branch that imported a module which
never existed. The single implementation in `crypt_core`
(`compute_hash_independent` / `compute_kdf_independent`) now serves both modes
for all formats, so the two paths cannot drift again.
"""
