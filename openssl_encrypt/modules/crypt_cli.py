...
def load_template_file(template_name: str) -> Optional[Dict[str, Any]]:
    ...
    # SECURITY: a group/other-writable template dir lets another local user
    # plant a template this path would then apply and downgrade key derivation
    # (gitlab#169). Strip those write bits best-effort before reading (read is
    # preserved for multi-user installs; a read-only package dir stays as-is).
...
def main_with_args(args=None):
    ...
    if args.action == "create-usb":
        try:
            ...
            # applied. That option does not exist on this branch -- it was
            # removed as legacy, and test_legacy_removal pins its absence from
            # this file -- so there is nothing to refuse (gitlab#205).
        ...
        if encryption_info and encryption_info["type"] == "asymmetric":
            ...
            if len(matching) == 0:
                ...
                for fp in encryption_info["recipient_fingerprints"]:
                    ...
                    print(f"  • {sanitize_for_display(fp)}", file=sys.stderr)
                eprint(
    ...
    try:
        ...
        if args.action == "encrypt":
            ...
            # Resolved ONCE, before the overwrite branch, so both output paths
            # use the same keypair. This used to be duplicated: the overwrite
            # branch had its own copy (which wrote the private key in the clear
            # and could not read a wrapped keyfile), and this one was labelled
            # "for non-overwriting case". Deleting the duplicate without moving
            # this left the overwrite path with no keyfile handling at all -- it
            # encrypted with an ephemeral key and only reached this block
            # afterwards, so a bad keyfile was reported after the input had
            # already been replaced (gitlab#157).
            #
            # `secrets` is imported locally further down this function, which
            # makes the name function-local everywhere in it -- so reading it
            # from here, above that import, raises UnboundLocalError rather
            # than falling back to the module-level import. Re-imported here
            ...
            if args.algorithm in [
                ...
                if getattr(args, "pqc_gen_key", False) and getattr(args, "pqc_keyfile", None):
                    ...
                    # create_secure_file only: PermissionLevel is already imported
                    # at module level, and re-importing it HERE would make the name
                    # function-local for the whole of main_with_args -- so the uses
                    # ABOVE this point (the stdin temp file, the dearmor path) would
                    # raise UnboundLocalError. Found by the armor tests.
                    from .file_permissions import create_secure_file
            ...
            if args.overwrite:
                ...
                try:
                    ...
                    # pqc_keypair is deliberately NOT reset here: it is resolved
                    # once above, before this branch, so an --overwrite run uses
                    # the same keyfile the non-overwrite path would. A second copy
                    # of the keyfile logic used to live here and wrote
                    # `private_key` as bare base64 with no key_encrypted marker --
                    # reintroduced by a file-reconstruction commit after the
                    # wrapping fix had landed, and missed by the later Argon2id
                    # upgrade, whose message says it touched "the one write site".
                    # Its loader read `private_key` unconditionally too, so given a
            ...
            try:
                ...
                # sign_file_cli exits directly on refusal (e.g. unknown
                # signer); a JSON caller still gets its one document
                # (gitlab#270 F4).
