...
def encrypt_file(
    ...
    # Extract keystore_id from hash_config once, ahead of the streaming branch —
    # the streaming and non-streaming metadata sites must never diverge on it
    # again (gitlab#275 happened because the streaming site drifted).
    keystore_id = hash_config.get("pqc_keystore_key_id") if isinstance(hash_config, dict) else None
...
def list_recovery_slots(input_file: str) -> list:
    ...
    Returns:
        ...
        ``num_shares`` are present only for shamir slots (creatable on the
        1.5.x line, listable here) whose header values validate as ints with
        ``2 <= K <= N <= 255``; malformed values are omitted, never echoed.
        All values come from the unauthenticated plaintext header and are
        verified only when the file is decrypted.
        ...
        if slot.get("type") == "shamir":
            ...
            if isinstance(shamir, dict):
                ...
                if (
                    ...
                    # Mirror the 1.5.x creation invariant (split_secret /
                    # _parse_k_of_n there; shamir slots cannot be created on
                    # this line but 1.5.x-authored files are listable): a
                    # crafted header must not render an impossible policy
                    # like "255 of 1".
