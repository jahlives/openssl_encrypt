...
def encrypt_file(
    ...
    if _use_streaming and xor_mode == "sequential":
    ...
    if _use_streaming and format_version not in (12, LATEST_STABLE_FORMAT_VERSION):
        # Streaming pins the format version before key derivation so the
        # decrypt-side key matches. Requests get the latest version so
        # streaming PQC files carry the v14 transcript binding; an explicit
        # format_version=12 request keeps writing v12.
        if debug:
            logger.debug(
                f"STREAMING: forcing format_version {format_version} -> "
                f"{LATEST_STABLE_FORMAT_VERSION} before key derivation so the "
                f"decrypt-side key matches"
            )
    ...
    if envelope or recovery_credentials:
        ...
        try:
            ...
            # Done here while the DEK (_dek) is still available.
    ...
    if _use_streaming:
        ...
        # Streaming pins the format version at the force site above; an
        # explicit v12 request keeps v12, everything else gets the latest.
...
def _reconstruct_cli_from_metadata(
    ...
    # Hidden ("whitened") format flags. --hidden-header reproduces the format on
    # any line (it is a no-op default on 1.5.x); keyed files additionally need
    # the second password (shown as the safe prompt form).
    if keyed:
...
def decrypt_file(
    ...
    if archive_info and archive_info.get("type") == "tar":
        ...
        if not quiet:
            eprint(f"Directory extracted to: {output_dir}")
    else:
