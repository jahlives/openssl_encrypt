...
REQUIRED_COMMANDS = {"encrypt", "decrypt", "shred", "rekey", "identity"}
...
KNOWN_BROKEN = [
    ...
    # planned (gitlab#194). Until it lands these calls fail at argparse, so
    # they are declared here rather than silently tolerated -- and the
    # stale-entry test will demand these entries be deleted the day the
    # subcommands appear.
...
class TestGuiArgvMatchesCli(unittest.TestCase):
    ...
    def _absorbs_a_positional(self, path):
        """Whether this command declares a positional that takes a value.

        If it does, a stray value after a zero-argument flag is consumed by
        that positional instead of failing, so the check cannot call it a
        defect. `generate-password` is the live example: it declares
        `[length]`, and the GUI's `--force-wordlist` and `length.toString()`
        sit in mutually exclusive if/else branches that this lint flattens
        into one argv anyway.
        """
        node = _resolve(self.parser, path)
        if node is None:
            return False
    ...
    def test_no_flag_is_given_a_value_it_cannot_take(self):
        ...
        Two limits, both deliberate. Commands that declare a positional are
        skipped: the value is absorbed rather than rejected, so it is not
        provably a defect. And this lint flattens control flow -- two
        `args.add` calls in mutually exclusive if/else branches read as
        adjacent -- so the positional skip also covers the false positive
        that flattening produces for `generate-password`.
        """
        problems = []
        for name, path, _flags, unresolved, _unverified, argv in self.sites:
            ...
            if unresolved is not None and unresolved in known:
                continue
            if self._absorbs_a_positional(path):
                # A stray value is taken by the positional rather than
                # rejected, so nothing here is provably wrong.
                continue
