...
never noticed and verification still passed. The fix records a v2 manifest that
also protects the root autorun files, and verification now flags added
executable-type files and tampered/added autorun files — while deliberately NOT
flagging user-added ``*.encrypted`` data (the normal use case).
...
class TestUsbIntegrityF13(unittest.TestCase):
    ...
    def test_legacy_v1_manifest_skips_added_detection(self):
        ...
        # Hand-build a legacy manifest (no scan_version / root_checksums).
        checksums = {}
