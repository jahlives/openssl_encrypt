...
class TestUnlockingAnExistingSlotDoesNotEnforce(unittest.TestCase):
    ...
    def test_a_weak_passphrase_still_unlocks(self):
        # No environ patch: the helper takes env_value as a parameter and
        # never reads os.environ, so patching it would only make this look
        # like it covers the env plumbing.
        value = recovery_slots._read_recovery_passphrase(
            "OPENSSL_ENCRYPT_RECOVERY_PASSPHRASE", WEAK, "Recovery passphrase: "
        )
        self.assertEqual(value, WEAK)

    def test_a_blank_passphrase_is_still_refused_on_unlock(self):
        """Blank stays refused: it cannot be a real slot's credential, so it
        is a mistake rather than a weak choice."""
        with self.assertRaises(ValueError):
            recovery_slots._read_recovery_passphrase(
                "OPENSSL_ENCRYPT_RECOVERY_PASSPHRASE", "  ", "Recovery passphrase: "
            )
...
class TestTheAddPathBehaves(unittest.TestCase):
    ...
    def test_a_weak_passphrase_creates_no_slot(self):
        ...
        with mock.patch.dict(os.environ, {"OPENSSL_ENCRYPT_ADD_RECOVERY_PASSPHRASE": WEAK}):
            with self.assertRaises(Exception):
    ...
    def test_force_password_creates_a_weak_slot_that_still_opens(self):
        ...
        with mock.patch.dict(os.environ, {"OPENSSL_ENCRYPT_RECOVERY_PASSPHRASE": WEAK}):
