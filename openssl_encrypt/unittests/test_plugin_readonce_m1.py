...
class TestM1ReadOnceBinding(unittest.TestCase):
    def setUp(self):
        ...
        self.plugin_manager = PluginManager(
            ...
            # This suite exercises the read-once/hash-pin binding that the
            # signature check performs, so the check must RUN — but on unsigned
            # test plugins. Use WARN (the old default) so the check runs and the
            # plugin still loads; ENFORCE (the new default, gitlab#130) would
            # refuse the unsigned plugin before the binding is exercised.
