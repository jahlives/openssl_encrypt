...
class TestTheRoutingSetIsDerived(unittest.TestCase):
    ...
    def test_the_routing_set_is_exactly_what_is_registered(self):
        """The cache must report the registry faithfully.

        The routing decision then intersects this with the known-command
        list, so a name the subparser does not register can never be routed.
        """
        from openssl_encrypt.modules.crypt_cli import _subparser_choices

        self.assertEqual(set(_subparser_choices()), _registered_choices())

    def test_the_previously_dead_seven_have_no_subparser(self):
        """They are handled by the monolithic parser, which declares them.

        On this branch the routing set is the INTERSECTION of the known-command
        list and what the subparser registers, so absence from the registry is
        what keeps them on the working path.
        """
        from openssl_encrypt.modules.crypt_cli import _subparser_choices
