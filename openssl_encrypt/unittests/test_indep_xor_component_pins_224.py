...
- golden keys for a one-of-each config on v13 and v14. They were computed
  ...
  salts, seed encoding, normalization or XOR breaks loudly. The no-whirlpool
  goldens are the cross-line pair (1.5.x has no whirlpool); the
  with-whirlpool goldens are 1.4.x-only.
"""
...
GOLDEN = {
    # (whirlpool_enabled, format_version): expected aes-256-gcm key hex
    (False, 13): "9e655e48076b78753de23b7e162bc2b697333bd7a6adf5194c89c2e1313db5e8",
    (False, 14): "58f2d50c577904dafaec5a0532ee58001a249bb9d33bbcd2612991d03cdcacfa",
    (True, 13): "8b8c8fb47772dc5b83a796de2b7c90d7aa9864fc0f0c8e0f30e853067b8798e8",
    (True, 14): "a845dcb7a20e6de9834465c00ab63d8ba09f1199bdcb4f1c8d05345c57283338",
}


def _config(whirlpool=False, randomx=False):
    cfg = {
        "sha256": 5,
        "sha512": 5,
        "sha3_256": 5,
            ...
            with mock.patch.object(cc, "compute_hash_independent", fake):
                with mock.patch.object(cc, "compute_kdf_independent", fake):
                    generate_key_independent_xor(
                        PASSWORD,
                        SALT,
                        _config(whirlpool=True, randomx=True),
                        quiet=True,
                        format_version=14,
                    )
...
class TestGoldenKeys(unittest.TestCase):
    ...
    def _derive(self, whirlpool, fmt, parallel):
        return bytes(
            generate_key_independent_xor(
                PASSWORD,
                SALT,
                _config(whirlpool=whirlpool),
                quiet=True,
    ...
    def test_golden_v13_sequential(self):
        self.assertEqual(self._derive(False, 13, False), GOLDEN[(False, 13)])

    def test_golden_v14_sequential(self):
        self.assertEqual(self._derive(False, 14, False), GOLDEN[(False, 14)])

    def test_golden_v13_parallel(self):
        self.assertEqual(self._derive(False, 13, True), GOLDEN[(False, 13)])

    def test_golden_v14_parallel(self):
        self.assertEqual(self._derive(False, 14, True), GOLDEN[(False, 14)])

    @unittest.skipUnless(WHIRLPOOL_AVAILABLE, "whirlpool module not available")
    def test_golden_whirlpool_v13_both_modes(self):
        self.assertEqual(self._derive(True, 13, False), GOLDEN[(True, 13)])
        self.assertEqual(self._derive(True, 13, True), GOLDEN[(True, 13)])

    @unittest.skipUnless(WHIRLPOOL_AVAILABLE, "whirlpool module not available")
    def test_golden_whirlpool_v14_both_modes(self):
        self.assertEqual(self._derive(True, 14, False), GOLDEN[(True, 14)])
        self.assertEqual(self._derive(True, 14, True), GOLDEN[(True, 14)])
