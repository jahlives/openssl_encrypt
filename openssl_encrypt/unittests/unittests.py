...
@unittest.skip("Video steganography disabled due to compression incompatibility with LSB")
class TestVideoSteganography(unittest.TestCase):
    """Test suite for video steganography functionality."""
    ...
    def setUp(self):
        ...
        # Check if video steganography is available
        try:
            from modules.steganography import (
            ...
            self.video_available = is_video_steganography_available()
            self.mp4_available = is_mp4_steganography_available()
        except ImportError:
            self.video_available = False
            self.mp4_available = False
    ...
    def test_video_steganography_availability(self):
        """Test if video steganography components are available."""
        if not self.video_available:
            self.skipTest(
                "Video steganography not available - requires opencv-python and ffmpeg-python"
            )
        ...
        self.assertTrue(is_video_steganography_available())

        # Test that base class exists
        self.assertIsNotNone(VideoSteganographyBase)
        self.assertIsNotNone(VideoFormatError)

    def test_mp4_steganography_availability(self):
        """Test if MP4 steganography is available."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")

        from modules.steganography import MP4Steganography, is_mp4_steganography_available

        # Test availability
        self.assertTrue(is_mp4_steganography_available())

        # Test instantiation
        mp4_stego = MP4Steganography()
        self.assertIsNotNone(mp4_stego)
        self.assertEqual(mp4_stego.security_level, 1)
        self.assertEqual(mp4_stego.quality_preservation, 8)

    def test_mp4_test_video_creation(self):
        """Test MP4 test video creation functionality."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")

        from modules.steganography import create_mp4_test_video

        # Create small test video
        mp4_data = create_mp4_test_video(width=160, height=120, duration=1.0)
        self.assertIsInstance(mp4_data, bytes)
        self.assertGreater(len(mp4_data), 1000)  # Should be at least 1KB

        # Should start with MP4 signature
        self.assertIn(b"ftyp", mp4_data[:20])  # MP4 file type box

    def test_mp4_capacity_calculation(self):
        """Test MP4 steganography capacity calculation."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")

        from modules.steganography import MP4Steganography, create_mp4_test_video

        # Create test video
        mp4_data = create_mp4_test_video(width=320, height=240, duration=2.0)

        # Test capacity calculation
        mp4_stego = MP4Steganography(quality_preservation=8)
        capacity = mp4_stego.calculate_capacity(mp4_data)

        self.assertIsInstance(capacity, int)
        self.assertGreater(capacity, 100)  # Should have reasonable capacity

        # Higher quality preservation should reduce capacity
        mp4_stego_hq = MP4Steganography(quality_preservation=10)
        capacity_hq = mp4_stego_hq.calculate_capacity(mp4_data)
        self.assertLess(capacity_hq, capacity)

    def test_mp4_steganography_workflow(self):
        """Test complete MP4 steganography hide/extract workflow."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")
        ...
        # Create test video
        mp4_data = create_mp4_test_video(width=320, height=240, duration=3.0)

        # Test data to hide
        test_data = b"MP4 steganography test data - hiding in video frames!"

        # Initialize MP4 steganography
        mp4_stego = MP4Steganography(
            password="mp4_test_password", quality_preservation=7, temporal_spread=True
        )

        # Check capacity
        capacity = mp4_stego.calculate_capacity(mp4_data)
        self.assertGreater(capacity, len(test_data), "Test data too large for MP4 capacity")

        # Hide data
        stego_data = mp4_stego.hide_data(mp4_data, test_data)
        self.assertIsInstance(stego_data, bytes)
        self.assertGreater(len(stego_data), 1000)  # Should be reasonable size

        # Verify it's still a valid MP4
        self.assertIn(b"ftyp", stego_data[:20])

        # Extract data
        extracted_data = mp4_stego.extract_data(stego_data)
        self.assertEqual(test_data, extracted_data)

    def test_mp4_steganography_with_audio(self):
        """Test MP4 steganography with audio track."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")
        ...
        try:
            ...
            mp4_data = create_mp4_test_video(width=240, height=180, duration=2.0, with_audio=True)

            # Test data
            test_data = b"MP4 with audio test!"

            # Initialize with audio track usage
            mp4_stego = MP4Steganography(
                password="mp4_audio_test", use_audio_track=True, quality_preservation=8
            )

            # Test capacity (should be higher with audio)
            capacity = mp4_stego.calculate_capacity(mp4_data)
            self.assertGreater(capacity, len(test_data))
            ...
            # Audio creation might fail in some environments
            self.skipTest(f"MP4 with audio test failed: {e}")

    def test_mp4_different_quality_settings(self):
        """Test MP4 steganography with different quality preservation settings."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")
        ...
        # Create test video
        mp4_data = create_mp4_test_video(width=200, height=150, duration=2.0)
        test_data = b"Quality test data!"

        # Test different quality levels
        for quality in [6, 8, 10]:
            with self.subTest(quality=quality):
                mp4_stego = MP4Steganography(quality_preservation=quality)
    ...
    def test_mp4_password_protection(self):
        """Test MP4 steganography with password protection."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")
        ...
        # Create test video
        mp4_data = create_mp4_test_video(width=240, height=180, duration=1.5)
        test_data = b"Password protected data!"

        # Hide with password
        mp4_stego = MP4Steganography(password="secret_password_123")
        stego_data = mp4_stego.hide_data(mp4_data, test_data)
        ...
        # Try with wrong password (should not extract correctly)
        mp4_stego_wrong = MP4Steganography(password="wrong_password")
        wrong_extracted = mp4_stego_wrong.extract_data(stego_data)
        self.assertNotEqual(test_data, wrong_extracted)

    def test_mp4_temporal_spreading(self):
        """Test MP4 steganography with temporal data spreading."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")

        from modules.steganography import MP4Steganography, create_mp4_test_video

        # Create longer test video for temporal testing
        mp4_data = create_mp4_test_video(width=200, height=150, duration=3.0)
        test_data = b"Temporal spreading test data across multiple frames!"

        # Test with temporal spreading enabled
        mp4_stego_spread = MP4Steganography(temporal_spread=True)
        capacity_spread = mp4_stego_spread.calculate_capacity(mp4_data)

        # Test with temporal spreading disabled
        mp4_stego_no_spread = MP4Steganography(temporal_spread=False)
        capacity_no_spread = mp4_stego_no_spread.calculate_capacity(mp4_data)

        # Both should work, temporal spreading might have slightly different capacity
        self.assertGreater(capacity_spread, 0)
        self.assertGreater(capacity_no_spread, 0)

        # Test functionality with spreading
        if capacity_spread > len(test_data):
            stego_data = mp4_stego_spread.hide_data(mp4_data, test_data)
            extracted_data = mp4_stego_spread.extract_data(stego_data)
            self.assertEqual(test_data, extracted_data)

    def test_video_format_detection(self):
        """Test video format detection in transport layer."""
        if not self.video_available:
            self.skipTest("Video steganography not available")

        from modules.steganography import SteganographyTransport, create_mp4_test_video
        ...
        # Test format detection
        transport = SteganographyTransport()
        format_detected = transport._detect_media_format(mp4_data)
        self.assertEqual(format_detected, "MP4")

    def test_video_transport_integration(self):
        """Test video steganography through transport layer."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")
        ...
        # Create test video
        mp4_data = create_mp4_test_video(width=200, height=150, duration=2.0)
        test_data = b"Transport layer test!"

        # Test through transport layer
        transport = SteganographyTransport(method="lsb", video_quality=8, temporal_spread=True)

        # Create steganography instance
        transport._create_stego_instance("MP4")
        self.assertIsNotNone(transport.stego)

        # Test capacity and workflow
        capacity = transport.stego.calculate_capacity(mp4_data)
        self.assertGreater(capacity, len(test_data))

        stego_data = transport.stego.hide_data(mp4_data, test_data)
        extracted_data = transport.stego.extract_data(stego_data)
        self.assertEqual(test_data, extracted_data)

    def test_video_steganography_error_handling(self):
        """Test video steganography error handling."""
        if not self.video_available:
            self.skipTest("Video steganography not available")

        from modules.steganography import MP4Steganography, VideoFormatError

        mp4_stego = MP4Steganography()

        # Test with invalid video data
        with self.assertRaises((VideoFormatError, Exception)):
            mp4_stego.calculate_capacity(b"invalid video data")

        # Test with empty data
        with self.assertRaises((VideoFormatError, Exception)):
            mp4_stego.calculate_capacity(b"")

    def test_large_data_handling(self):
        """Test handling of data that exceeds video capacity."""
        if not self.mp4_available:
            self.skipTest("MP4 steganography not available")

        from modules.steganography import CapacityError, MP4Steganography, create_mp4_test_video

        # Create small test video
        mp4_data = create_mp4_test_video(width=100, height=100, duration=0.5)

        mp4_stego = MP4Steganography()
        capacity = mp4_stego.calculate_capacity(mp4_data)

        # Try to hide data larger than capacity
        large_data = b"X" * (capacity + 1000)

        with self.assertRaises(CapacityError) as context:
            mp4_stego.hide_data(mp4_data, large_data)

        self.assertIn("capacity", str(context.exception).lower())


@unittest.skip("Video steganography disabled due to compression incompatibility with LSB")
class TestVideoUtilities(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Check if video utilities are available
        try:
            from modules.steganography.video_utils import is_video_utils_available

            self.utils_available = is_video_utils_available()
        except ImportError:
    ...
    def test_video_validation(self):
        ...
        if not self.utils_available:
            self.skipTest("Video utilities not available")

        from modules.steganography import create_mp4_test_video
        from modules.steganography.video_utils import validate_video_file

        # Create test video
        mp4_data = create_mp4_test_video(width=160, height=120, duration=1.0)

        # Validate video
        validation = validate_video_file(mp4_data)

        self.assertIsInstance(validation, dict)
        self.assertTrue(validation.get("valid", False))
        self.assertIn("video", validation)
        self.assertGreater(validation["video"]["width"], 0)
        self.assertGreater(validation["video"]["height"], 0)

    def test_frame_complexity_calculation(self):
        """Test frame complexity calculation."""
        if not self.utils_available:
            self.skipTest("Video utilities not available")

        import numpy as np
        from modules.steganography.video_utils import calculate_frame_complexity

        # Create test frames with different complexity levels
        # Simple frame (low complexity)
        simple_frame = np.zeros((100, 100, 3), dtype=np.uint8)
        simple_complexity = calculate_frame_complexity(simple_frame)

        # Complex frame (high complexity)
        complex_frame = np.random.randint(0, 256, (100, 100, 3), dtype=np.uint8)
        complex_complexity = calculate_frame_complexity(complex_frame)

        # Verify structure
        for complexity in [simple_complexity, complex_complexity]:
            self.assertIn("variance", complexity)
            self.assertIn("complexity_score", complexity)
            self.assertIn("edge_density", complexity)
            self.assertIsInstance(complexity["complexity_score"], float)

        # Complex frame should have higher complexity score
        self.assertGreater(
            complex_complexity["complexity_score"], simple_complexity["complexity_score"]
        )

    def test_capacity_estimation(self):
        """Test video capacity estimation."""
        if not self.utils_available:
            self.skipTest("Video utilities not available")

        from modules.steganography import create_mp4_test_video
        from modules.steganography.video_utils import estimate_video_capacity, validate_video_file

        # Create test video
        mp4_data = create_mp4_test_video(width=200, height=150, duration=2.0)

        # Get video info
        video_info = validate_video_file(mp4_data)

        # Estimate capacity
        capacity = estimate_video_capacity(video_info, bits_per_pixel=1.0)

        self.assertIsInstance(capacity, dict)
        self.assertIn("total_capacity", capacity)
        self.assertIn("video_capacity", capacity)
        self.assertGreater(capacity["total_capacity"], 0)
        self.assertGreater(capacity["video_capacity"], 0)

    def test_psnr_calculation(self):
        """Test PSNR calculation between frames."""
        if not self.utils_available:
            self.skipTest("Video utilities not available")

        import numpy as np
        from modules.steganography.video_utils import calculate_psnr

        # Create test frames
        original = np.random.randint(0, 256, (50, 50, 3), dtype=np.uint8)

        # Identical frame should have infinite PSNR
        psnr_identical = calculate_psnr(original, original.copy())
        self.assertEqual(psnr_identical, float("inf"))

        # Slightly modified frame should have high PSNR
        modified = original.copy()
        modified[0, 0, 0] = (modified[0, 0, 0] + 1) % 256  # Change one pixel slightly
        psnr_modified = calculate_psnr(original, modified)

        self.assertIsInstance(psnr_modified, float)
        self.assertGreater(psnr_modified, 30)  # Should be high quality
