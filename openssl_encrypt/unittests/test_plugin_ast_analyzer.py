...
"""
    ...
    def test_os_forkpty_detected(self):
        """os.forkpty() should be blocked"""
        code = """
import os
...
"""
    ...
    def test_loader_dunder_access_detected(self):
        """__loader__ and __spec__ access can be used to bypass import restrictions"""
        code = """
def test():
    loader = __loader__
    return loader
"""
        # __loader__ as a Name won't trigger dunder_access (it's an attribute check),
        # but accessing it on an object should
        code2 = """
def test(mod):
    return mod.__loader__
"""
        is_safe, violations = analyze_plugin_code(code2, "test.py", strict_mode=True)

        assert not is_safe
        assert any(v.violation_type == "dunder_access" for v in violations)

    def test_import_dunder_attribute_detected(self):
        """__import__ as attribute access should be blocked"""
        code = """
def test():
...
"""
    ...
    def test_class_bases_subclasses_chain_detected(self):
        """Full sandbox escape chain: ().__class__.__bases__[0].__subclasses__()"""
        code = """
def test():
...
"""
        ...
        assert not is_safe
        # Multiple violations: __class__, __bases__, __subclasses__ (dunder_access),
        # plus __subclasses__() call
