#!/usr/bin/env python3
"""list-recovery exposes shamir K-of-N parameters (gitlab#278).
...
raw metadata itself. The fields are untrusted header input: they are only
passed through when they validate as ints with 2 <= K <= N <= 255 (the
creation invariant of split_secret/_parse_k_of_n), and are omitted otherwise.
"""
...
class ShamirListingBase(unittest.TestCase):
    ...
    def _encrypt_with_shamir(self, spec="2-of-3"):
        data = encrypt_file(
...
class TestShamirKofNExposed(ShamirListingBase):
    def test_shamir_slot_reports_threshold_and_num_shares(self):
        self._encrypt_with_shamir("2-of-3")
    ...
    def test_core_listing_carries_the_fields(self):
        self._encrypt_with_shamir("3-of-5")
        shamir = [s for s in list_recovery_slots(self.enc) if s["type"] == "shamir"][0]
        self.assertEqual(shamir["threshold"], 3)
        self.assertEqual(shamir["num_shares"], 5)
    ...
    def test_shamir_slot_key_id_matches_share_set(self):
        """The slot stores the share-set UUID split_secret stamped on disk."""
        self._encrypt_with_shamir("2-of-3")
        shamir = [s for s in self._list_json()["slots"] if s["type"] == "shamir"][0]
        share_path = os.path.join(self.tmp, "shares", "recovery_share_1.json")
        with open(share_path) as f:
            share_key_id = json.load(f)["metadata"]["key_id"]
        self.assertEqual(shamir["key_id"], share_key_id)
...
class TestMalformedShamirParams(unittest.TestCase):
    ...
    def test_out_of_range_values_are_omitted(self):
        for params in (
            ...
            # (split_secret and _parse_k_of_n both reject it), so a crafted
            # header must not render "5 of 2".
    ...
    def test_cli_paths_omit_malformed_values_end_to_end(self):
        ...
        import openssl_encrypt.modules.crypt_core as cc
        from openssl_encrypt.modules.recovery_slots import list_recovery_cli
...
class TestMalformedHeaderShapes(unittest.TestCase):
    ...
    def test_huge_numeric_literal_in_raw_header_is_handled(self):
        ...
        with open(path, "wb") as f:
            f.write(b64.b64encode(raw_meta) + b":payload")
        try:
            try:
