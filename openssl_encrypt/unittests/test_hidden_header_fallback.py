...
class TestSecondPasswordFallback(unittest.TestCase):
    ...
    def test_keyed_prompts_when_stdin_piped_but_terminal_present(self):
        ...
        tty = mock.patch("openssl_encrypt.modules.crypt_cli._can_prompt_on_tty", return_value=True)
        stdin = mock.patch("openssl_encrypt.modules.crypt_cli.sys.stdin")
        gp = mock.patch("getpass.getpass", return_value="entered-pw")
        with tty, stdin as si, gp as gp_mock:
            si.isatty.return_value = False  # piped ciphertext on stdin
            out = _resolve_second_password_with_fallback(_args(self.keyed), None)
        self.assertEqual(out, b"entered-pw")
...
class TestArmoredStdinKeyedDecrypt(unittest.TestCase):
    ...
    def _run_decrypt_stdin(self, payload, second_pw):
        ...
        _sys.stdout = devnull
        p_stdin = mock.patch("openssl_encrypt.modules.crypt_cli.sys.stdin", fake_stdin)
        p_tty = mock.patch(
            "openssl_encrypt.modules.crypt_cli._can_prompt_on_tty", return_value=True
        )
        p_getpass = mock.patch("getpass.getpass", return_value=second_pw)
        p_exit = mock.patch("sys.exit", side_effect=lambda c=0: codes.append(c))
        try:
            with p_stdin, p_tty, p_getpass, p_exit:
                cli_main()
        finally:
...
class TestCanPromptOnTty(unittest.TestCase):
    ...
    def test_true_when_dev_tty_openable(self):
        ...
        p_open = mock.patch("openssl_encrypt.modules.crypt_cli.os.open", return_value=3)
        p_close = mock.patch("openssl_encrypt.modules.crypt_cli.os.close")
        with p_open, p_close:
    ...
    def test_false_when_no_tty_anywhere(self):
        ...
        p_open = mock.patch("openssl_encrypt.modules.crypt_cli.os.open", side_effect=OSError)
        p_in = mock.patch("openssl_encrypt.modules.crypt_cli.sys.stdin")
        p_out = mock.patch("openssl_encrypt.modules.crypt_cli.sys.stdout")
        p_err = mock.patch("openssl_encrypt.modules.crypt_cli.sys.stderr")
        with p_open, p_in as si, p_out as so, p_err as se:
