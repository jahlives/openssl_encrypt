...
class TestImportDisplaySanitized(_CmdTestBase):
    """1.5.x cmd_import is the pre-gitlab#164 file-based surface: no
    --data-stdin, no --alias."""

    def _run_import(self, identity, document):
        import tempfile
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
            ...
            path = f.name
        args = argparse.Namespace(file=path, overwrite=False, identity_store=None)
        with mock.patch(
            "openssl_encrypt.modules.identity.Identity.import_public",
            return_value=identity,
        ), mock.patch("openssl_encrypt.modules.identity_cli.get_identity_store"):
    ...
    def test_import_success_output_carries_no_raw_escapes(self):
        ...
        status, output = self._run_import(self._identity(FORGED), {"name": "mallory"})
