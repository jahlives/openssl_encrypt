...
# lib/widgets/ or lib/screens/, so 13 of the 14 entries here described
...
KNOWN_UNWIRED = [
    ...
    # gitlab#198: deliberate — independent XOR is already the CLI's default
    # composition (verified in cli_service.dart:440-444), so an explicit
    # control would be redundant surface. The parameter stays for the
    # sequential-vs-independent mutual-exclusion guard.
    (
        "encryptTextWithProgress",
        "independentXor",
        "gitlab#198: redundant with the default composition",
    ),
]
