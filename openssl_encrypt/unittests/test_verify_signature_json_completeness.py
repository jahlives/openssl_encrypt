...
class _SignatureTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        result = self._cli(
            # --no-armor so the sidecar is raw JSON these tests can read and
            # tamper with; the armored form is the shipped default.
            "sign",
            "-i",
            str(self.target),
            "--sign-with",
    ...
    def _cli(self, *argv):
        env = dict(os.environ)
        env["OPENSSL_ENCRYPT_SIGNER_PASSPHRASE"] = self.PW
