#!/usr/bin/env python3
"""Golden-fixture tests: committed recovery-slot files (1.4.x port, no Shamir)
must keep decrypting with their fixed recovery credentials, pinning the on-disk
format. Generated with a portable SHA-256-only KDF."""
...
class TestRecoverySlotGoldenFixtures(unittest.TestCase):
    def test_fixtures_have_slot_set(self):
        for name, typ in (("recovery_code.enc", "recovery_code"), ("passphrase.enc", "passphrase")):
