...
class FileManager {
  ...
  /// Get list of bundled test files
  Future<List<String>> getTestFileNames() async {
    final List<String> testFiles = [
      // V3 format test files (encrypted)
      'v3/test1_fernet.txt',
      'v3/test1_aes-gcm.txt',
      ...
      'v5/test1_xchacha20-poly1305.txt',
      'v5/test1_fernet_balloon.txt',
  ...
  /// Read test file content from assets
  Future<String?> readTestFile(String fileName) async {
    try {
      final String content = await rootBundle.loadString('assets/test_files/$fileName');
      return content;
    } catch (e) {
      CLIService.outputDebugLog('Error reading test file $fileName: $e');
      return null;
    }
  }

  /// Create a FileInfo object for a test file
  Future<FileInfo?> getTestFileInfo(String fileName) async {
    try {
      final String content = await rootBundle.loadString('assets/test_files/$fileName');
      ...
      return FileInfo(
        ...
        size: bytes,
        extension: path.extension(fileName).toLowerCase(),
        lastModified: DateTime.now(), // Use current time for bundled files
      );
    } catch (e) {
      CLIService.outputDebugLog('Error getting test file info for $fileName: $e');
      return null;
    }
  }
}
