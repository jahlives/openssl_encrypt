...
my_array=("aes-gcm" "aes-gcm-siv" "aes-siv" "chacha20-poly1305" "xchacha20-poly1305")
...
for FILE in test1_aes-gcm-siv.txt  test1_chacha20-poly1305.txt  test1_fernet.txt     test1_ml-kem-512.txt  test1_xchacha20-poly1305.txt test1_aes-gcm.txt      test1_aes-siv.txt   test1_fernet_balloon.txt     test1_ml-kem-1024.txt  test1_ml-kem-768.txt ; do
  ALGO=${FILE}
  ALGO=${ALGO#test1_}
  ALGO=${ALGO%.txt}
  END=''
  ...
  python -m openssl_encrypt.crypt encrypt -i /tmp/test_input.txt -o ${OUTPUT}/${FILE} \
    ...
    --enable-argon2 --argon2-rounds 10 ${ALGO_Q} ${END}
