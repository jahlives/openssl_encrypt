...
- **Batch-tab hardening follow-ups from the #214 review** (gitlab#215):
  ...
  algorithm, hash/KDF config, recipients, signer, cascade, HSM, integrity
  settings — previously half were read live mid-run, so editing a control
  changed how the remaining files were processed); a class-level run guard
  refuses a second concurrent batch after the tab was left and re-entered
  mid-run; the Decrypt tab distinguishes an identity-store **load failure**
  from an empty store and surfaces store entries the CLI reported as
  unreadable; merged signer dropdown entries carry their `(own)`/`(contact)`
  source tag so a name collision stays visible (#173 surface); the
  debug-window command mask covers every secret-valued flag instead of only
  `--password`. A widget test drives a full 2-file batch decrypt through
  the fake CLI seam and pins that every argv keeps `--no-verify` (never
  `--verify-from`) and every results row carries the "verification
  SKIPPED" marker.
...
- **`list-recovery` hardening batch — 1.4.x parity port**
  (gitlab#279/#280, security review of the gitlab#278 work): the
  credential-free listing is pinned and bounded end to end.
  *Payload pinning:* the `--json` per-slot document is filtered fail-closed
  through a module-level `SLOT_DOC_KEYS` allowlist and the data document's
  top level through `LISTING_DOC_KEYS` — growing the listing's output now
  always requires a visible edit to a pinned constant, never a silent
  ride-along inside the builder. *Boundary re-validation:* both the JSON
  and human views re-validate the shamir K-of-N values against the full
  `2 <= K <= N <= 255` creation invariant (bools excluded, pair ordering
  checked) instead of trusting the core listing. *Unauthenticated marker:*
  the `--json` data document opens with `"metadata_authenticated": false`
  so a GUI can gate destructive advice (e.g. "discard surplus shares") on
  the fact that a tampered header could under-report N; declared with
  `truncated` in the capabilities manifest `json_fields` (schema identical
  to 1.4.x). *Uniform container validation:* all `dek_slots` readers
  (list, add, remove, `_recover_envelope_dek`, and the decrypt-time
  recovery branch) share one validated accessor — crafted container shapes
  fail as `ValidationError` instead of raw internal `AttributeError`s, the
  write paths refuse a crafted container before the primary password is
  consumed, and only an absent/None section or `{}` means "no slots".
  *key_id normalization:* the type-native `params.shamir.key_id` wins over
  a top-level `params.key_id` and empty strings are omitted symmetrically.
  *Bounded reads and caps:* `_read_envelope_file` gains a `header_only`
  chunked read with a 2 MB metadata cap used by the listing (full mode
  enforces the same header cap; `RecursionError` from a JSON nesting bomb
  is normalized), `list_recovery_slots` materializes at most
  `MAX_DEK_SLOTS + 1` summaries, and a truncated human listing says "more
  than 32 recovery slot(s)" instead of presenting the capped length as the
  file's slot count. *One-document guard:* `json_output.mark_emitted()`
  lets bare-document endpoints (`identity list`) record emission so the
  dispatch-level `document_emitted()` guard is truthful. Pinned by
  `test_stdout_payload_pinning_280.py`, `test_dek_slot_container_280.py`,
  `test_list_recovery_caps_279.py`, and a new real-producer golden
  (`shamir_keyid.enc` + shares, generated by this line's
  `add-recovery --add-shares 2-of-3`) asserting the `key_id`-stamped
  listing and share-based decrypt.
...
- **Published package metadata no longer derives from the pip-compile
  ...
  `requirements-prod.txt` line-by-line, which let the aarch64 RandomX
  fork's direct-URL git requirement (gitlab#282) into `Requires-Dist` —
  PyPI rejects such uploads outright, and the fork would have become
  install-time trust baked into the published metadata of every aarch64
  install (the 1.4.x line's parser additionally leaked pip-compile's
  indented `# via ...` comments into the metadata; this line's parser
  already stripped them). Dependencies are now declared explicitly in
  `setup.py` (RandomX from PyPI only, scoped `platform_machine !=
  "aarch64"` where that build cannot import; arm64 pip users install the
  commit-pinned fork with the one-liner documented in the README), the
  dev/hsm extras parse their requirements files with comment- and URL-safe
  filtering, and a new metadata test suite pins the contract: every
  declared requirement parses as PEP 508, none is a direct URL, RandomX
  stays scoped off aarch64, and the declared set cannot drift from
...
- **The orphan-password NOTE now fires on every incomplete encrypt exit,
  ...
  disk* (set the moment the ciphertext is written, before armor
  ...
  "verify before removing" whenever an output may exist (e.g. an armor
  rewrite failure after the ciphertext was written — telling the user "you
  can remove it" there would delete the only credential of a live
  encrypted file) and "you can remove it" only when provably none
...
- **Parallel-KDF truthfulness and pinning follow-ups** (gitlab#224 items
  ...
  imports (blake3, argon2, balloon, randomx — the last runs subprocess
  availability probes at import) are resolved on the main thread before
  workers start. New golden pins cover the component-salt names and a
  one-of-each-component key for v13/v14 in both modes, cross-line pinned
  against the 1.4.x counterpart — previously only sha256/argon2/scrypt had
  goldens, so a typo in any other component's domain-separation name would
  have changed derived keys undetected. Stale
...
- **Decrypt enforces a hard KDF time ceiling, not just a memory ceiling**
  ...
  `--quiet`/`--no-estimate` display flags. The estimator was also corrected to
  scale Argon2 time by `memory_cost` and scrypt time by the full `N·r·p` work
  product, closing a memory×time cross-term that let a config sit under the
  memory ceiling yet run for hours. Found by the 1.4.9 pre-release scan.
...
- **Plaintext-hash confirmation oracle removed from the file header**
  ...
  (AEAD tag / Fernet HMAC / streaming trailer HMAC / v7 signed header). It is no
  longer written on any path and the metadata schemas no longer require it;
  decrypt stays tolerant of older files that still carry it (presence-guarded).
...
- **Identity loading now re-derives the fingerprint from the keys and fails
  ...
  warning. `Identity.load` now calls `check_fingerprint_consistency()` (over the
  cheap public keys, before the private-key KDF) and refuses to load an entry
  whose stored fingerprint does not match the public keys on disk (so
  `list_identities` skips and reports it), and the `add_identity` TOFU
  key-change check now compares the **recomputed** fingerprint on both sides so
  a substitution is detected from the actual keys. Found by the 1.4.9
  pre-release security scan.
...
- **Dependency updates for published CVEs** (Dependabot; patched versions
  ...
  - dev-only: `black` 26.1.0 → 26.3.1 — CVE-2026-32274 (arbitrary file write
    via an unsanitized cache filename); dev-time formatter, coordinated with
    the `.pre-commit-config.yaml` rev, and the tree was reformatted to match.
