            ...
            try:
                ...
                try:
                    ...
                    if actual_private_size >= 16:
                        print(
                            f"  ✓ AES-128: Compatible (needs 16 bytes, has {actual_private_size})"
                        )
                    else:
                        print(
                            f"  ✗ AES-128: Not compatible (needs 16 bytes, has {actual_private_size})"
                        )

                    if actual_private_size >= 24:
                        print(
                            f"  ✓ AES-192: Compatible (needs 24 bytes, has {actual_private_size})"
                        )
                    else:
                        print(
                            f"  ✗ AES-192: Not compatible (needs 24 bytes, has {actual_private_size})"
                        )

                    if actual_private_size >= 32:
                        print(
                            f"  ✓ AES-256: Compatible (needs 32 bytes, has {actual_private_size})"
                        )
                    else:
                        print(
                            f"  ✗ AES-256: Not compatible (needs 32 bytes, has {actual_private_size})"
                        )
