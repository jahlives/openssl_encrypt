...
This module provides signature verification using liboqs (Open Quantum Safe).
...
def verify_bundle_signature(bundle_data: Dict) -> bool:
    ...
    Args:
        bundle_data: Dictionary with bundle data (from KeyBundleSchema)
...
def calculate_fingerprint(key_data: bytes) -> str:
    ...
    This must match the client-side fingerprint calculation exactly.
...
def verify_revocation_signature(
    ...
    The revocation signature should be a signature of the fingerprint
    using the key being revoked (proof of ownership).
