...
if [[ "$FORCE_CLEAN" == "true" ]]; then
    ...
    flatpak-builder --disable-rofiles-fuse --disable-sandbox --repo=repo --default-branch="$FLATPAK_BRANCH" build-dir com.opensslencrypt.OpenSSLEncrypt.json
else
    ...
    flatpak-builder --disable-rofiles-fuse --disable-sandbox --force-clean --repo=repo --default-branch="$FLATPAK_BRANCH" build-dir com.opensslencrypt.OpenSSLEncrypt.json
fi
