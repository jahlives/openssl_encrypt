...
try:
    from modules.crypt_core import (
        ...
        decrypt_file_debug, DEBUG_MODE, generate_key, multi_hash_password
)
...
class TestEncryptionAlgorithms(unittest.TestCase):
    ...
    def test_existing_decryption(self):
        for type in ['fernet']:
            with open('./test/test1_' + type + '.txt', 'rb') as file:
                file_content = file.read()

            # Split into components
            metadata_b64, algo, encrypted = file_content.split(b':')
            ...
            key = generate_key(hashed_password)[0]
            print(f"Generated key (base64): {base64.b64encode(key).decode('utf-8')}")

            # Reconstruct the encrypted data with algorithm prefix
            full_encrypted = b':'.join([metadata_b64, algo, encrypted])  # Try this instead


            # Move the assertion inside the try block and ensure we fail on exception
            try:
                decrypted_data = decrypt_with_algorithm(full_encrypted, key)
...
class TestCommandLineInterfaceAdvanced(unittest.TestCase):
    ...
    def test_cli_wrong_password(self):
        ...
        self.assertTrue(
            "password" in stderr.lower() or "decryption failed" in stderr.lower() or "password" in stdout.lower() or "decryption failed" in stdout.lower())
