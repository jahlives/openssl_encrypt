...
def derive_key_from_password(password: str, method: str, params: Dict) -> bytes:
    ...
    if method == KeystoreProtectionMethod.ARGON2ID_AES_GCM.value:
        ...
        kdf = Scrypt(
            salt=salt, length=32, n=scrypt_params["n"], r=scrypt_params["r"], p=scrypt_params["p"]
        )
        ...
        kdf = PBKDF2HMAC(
            algorithm=hashes.SHA256(), length=32, salt=salt, iterations=pbkdf2_params["iterations"]
        )
        derived_key = kdf.derive(password.encode("utf-8"))
            ...
            try:
                ...
                try:
                    ...
                    plaintext = cipher.decrypt(
                        nonce, ciphertext, associated_data=header_associated_data
                    )
                    ...
                    try:
                        ...
                        print(
                            "Manual decrypt - Successfully decrypted with header_bytes as associated_data"
                        )
